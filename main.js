...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const slides = document.querySelector('.partner-slides');
    const slideItems = document.querySelectorAll('.partner-slide');
    ...
    // Event-Listener für Navigationselemente
    ...
    // Pause wenn Maus über dem Slider ist
...
function initCookieBanner() {
    ...
    if (localStorage.getItem('cookie-consent')) {
        ...
        // Checkboxen entsprechend der gespeicherten Einstellungen setzen
        if (analyticsCheckbox) analyticsCheckbox.checked = cookieSettings.analytics;
        if (marketingCheckbox) marketingCheckbox.checked = cookieSettings.marketing;
        
        // Cookie-Einstellungen anwenden
        applyCookieSettings(cookieSettings);
    } else {
        // Sofort anzeigen, keine Verzögerung
    ...
    // Event-Listener für den "Nur notwendige"-Button
    if (declineButton) {
        declineButton.addEventListener('click', function() {
            console.log('Ablehnen-Button wurde geklickt');
    ...
    // Hilfsfunktion zum Ausblenden des Banners
    function hideBanner() {
        cookieBanner.classList.add('hidden');
        setTimeout(() => {
            cookieBanner.style.display = 'none';
        }, 500); // Warte auf die Animation, bevor wir es vollständig ausblenden
    }
    ...
    // Hilfsfunktion zum Anwenden der Cookie-Einstellungen
    function applyCookieSettings(settings) {
        // Hier können die eigentlichen Cookie-Einstellungen implementiert werden
        // z.B. Analytics aktivieren/deaktivieren
        if (settings.analytics) {
            // Analytics-Tracking aktivieren
            console.log('Analytics-Tracking aktiviert');
        }
